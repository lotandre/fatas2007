...
class Decision(Page):
    ...
    def get_form_fields(self):
        ...
        if self.player.round_number == 1:
            formfield_list = ['start_pay1', 'check_start_pay1', 'fix_payoff1', 'bonus_payoff1']
        elif self.player.round_number == 16:
            formfield_list = ['start_pay2', 'check_start_pay2', 'fix_payoff2', 'bonus_payoff2']
        elif self.player.round_number == 31:
            formfield_list = ['start_pay3', 'check_start_pay3', 'fix_payoff3', 'bonus_payoff3']
    ...
    def before_next_page(self):
        if self.player.round_number == 1:
            ...
            self.participant.vars['bonus_payoff1'] = self.player.bonus_payoff1
        if self.player.round_number == 16:
            self.participant.vars['start_pay2'] = self.player.start_pay2
            self.participant.vars['fix_payoff2'] = self.player.fix_payoff2
            self.participant.vars['bonus_payoff2'] = self.player.bonus_payoff2
        if self.player.round_number == 31:
            ...
            self.participant.vars['bonus_payoff3'] = self.player.bonus_payoff3
...
class Draw(Page):
    ...
    def is_displayed(self):
        advance = ''
        if self.player.round_number < 16 and self.player.round_number <= self.participant.vars['red_period_comb1']:
            advance = 'yes'
        elif self.player.round_number in range(16, 31) and \
                self.player.round_number <= self.participant.vars['red_period_comb2'] + 15:
            advance = 'yes'
    ...
    def vars_for_template(self):
        ...
        if self.player.round_number < 16:
            ...
            bonus_payoff = self.participant.vars['bonus_payoff1']
            red_period = self.participant.vars['red_period_comb1']
        elif self.player.round_number in range(16, 31):
            start_pay = self.participant.vars['start_pay2']
            fix_payoff = self.participant.vars['fix_payoff2']
            bonus_payoff = self.participant.vars['bonus_payoff2']
            red_period = self.participant.vars['red_period_comb2']
        elif self.player.round_number > 30:
            start_pay = self.participant.vars['start_pay3']
            fix_payoff = self.participant.vars['fix_payoff3']
            bonus_payoff = self.participant.vars['bonus_payoff3']
            red_period = self.participant.vars['red_period_comb3']
